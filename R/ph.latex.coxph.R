ph.latex.coxph <- function(mod, variable, pairwise = TRUE, estimate = TRUE, se = FALSE,
  ...
  if (pairwise == TRUE) {
    ...
    colnames(coefem) <- c("Contrast", "Hazard Ratio", "Std. Error", "z-Ratio", "p-Value")
    ...
    if(is.null(title)) title <- "Pairwise Hazard Ratios"
  }
  
  if (pairwise == FALSE) {
    coefem <- data.frame(emmod)[, -4]
    if (is.null(varlab)) varlab <- variable
    colnames(coefem) <- c(varlab, "EM Hazard", "Std.Error", "Lower CL", "Upper CL")
    inc.col <- which(c(estimate, se, ci, ci) != 0) + 1
    coefem <- coefem[, c(1, inc.col), drop = FALSE]
    if(is.null(title)) title <- "EM Hazards"
  }
