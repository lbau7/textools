lm.latex.lme <- function(mod, results = c("summary", "Anova"), estimate = TRUE, ci = TRUE,
  ci.level = 0.95, se = FALSE, teststatistic = FALSE, df = TRUE, 
  pval = TRUE, intercept = FALSE, title = NULL, rowlabs = NULL, 
  ...
  if (results == "summary") {
    ...
    if (ci == TRUE & estimate == TRUE) {
      estci <- intervals(mod, level = ci.level)$fixed
  ...
  tabcaption <- paste0(title, " (n = ", nrow(mod$data), ")")
